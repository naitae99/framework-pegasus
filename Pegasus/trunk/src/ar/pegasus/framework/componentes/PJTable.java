...
import ar.pegasus.framework.util.Diccionario;
...
public class PJTable extends JTable implements ListSelectionListener, PJTableEventListener, TableColumnModelListener {
	...
	private int lastSortedColIndex = -1; //Variable que contiene el �ndice de la �ltima columna por la cual se ordeno el contenido de la tabla
	private int lastEditedRow = -1;
	private Object cellOldValue; //Variable que contiene el valor anterior de la celda que se est� editando
	private static int fontSize = 11; //Variable que contiene el tama�o de la fuente
	private List<GrupoCeldas> gruposCeldas; //Colecci�n que contiene los grupos de celdas
	 ...
	 */
	public PJTable(int rows, int cols) {
...
//		getInputMap().put(KeyStroke.getKeyStroke("shift ENTER"), "selectPreviousColumnCell");
		...
		setAutoResizeMode(PJTable.AUTO_RESIZE_OFF);
	...
	/**
	 * Retorna la m�scara para la fecha. Por defecto es <b>dd/mm/yyyy</b>.
	...
	/**
	 * Establece la m�scara para la fecha. Por defecto es <b>dd/MM/yyyy</b>.
	 * Hace c�lculos preliminares para utilizar m�s adelante.
	 * @param mask
	 */
	...
	/**
	 * Retorna la m�scara para la hora. Por defecto es <b>hh:mm:ss</b>.
	...
	/**
	 * Establece la m�scara para la hora: por defecto es <b>hh:mm:ss</b>. Hace
	 * c�lculos preliminares para utilizar m�s adelante.
	 * @param mask
	 */
	...
	/**
	 * Retorna si se le permitir� o no al usuario seleccionar las columnas visibles
	 * al hacer doble click en el header.
	 * @return <b>true</b> si se le permitir� al usuario seleccionar las columnas visibles.
	 */
	...
	/**
	 * Establece la cantidad de filas m�xima de la tabla. Si es necesario agrega
	...
	/** Detecci�n del evento de cambio de fila */
	...
	/**
	 * Agrega una fila vac�a a la tabla.
	 */
	public void addRow() {
...
//		if(rowData.size() != getColumnCount()) throw new IllegalArgumentException("El vector tiene dimensiones inv�lidas");
	...
	/**
	 * Retorna el tipo de columna en la posici�n <code>col</code>.
	 * @param col
	 * @return
	 */
	...
	/**
	 * Retorna el <b>ancho</b> de la columna en el �ndice pasado por par�metro.
	 * @param index
	 * @return
	 */
	 ...
	 * pertenece a una columna del tipo <b>DateColumn</b> el m�todo devolver�
        ...
        if(value instanceof String) {
            if(tipoColumna instanceof DateColumn || (celda != null && celda.getType() == DATE_CELL)) {
                ...
                if(value.toString().trim().compareTo("") != 0) {
                    Timestamp ts = stringToTimestamp(String.valueOf(value));
                    return ts;
                } else {
                    return null;
                }
	...
	/**
	 * Setea la alineaci�n de una celda.
	 * @param row
	 * @param col
	 * @param alignment
	 */
	...
	/**
	 * Setea el tipo de una celda espec�fica.
	 * @param row
	 * @param col
	 * @param type
	 * @param format
	...
	/**
	 * Retorna la alineaci�n de una celda.
	 * @param row
	 * @param col
	 * @return
	 */
	...
	public boolean inputTimeVerifier(char inputDateVerified, PJFormattedTextField ftf) {
	...
	/** Cancela la edici�n de una celda */
	 ...
	 * @param colClave La columna en la cual se encuentran las claves que identifican un�vocamente a cada fila.
	 */
	...
	//M�todos para accesos m�s r�pidos a determinados valores */
	...
	private class StringEditor extends KeyEditor {
		private static final long serialVersionUID = -5842772258814217311L;

		PJFormattedTextField ftf;
		StringListener stringListener;

		public StringEditor(PJTable t, int size) {
			super(t);
			ftf = (PJFormattedTextField)getComponent();
	...
	protected class IntegerEditor extends KeyEditor {
		...
		public IntegerEditor(PJTable t, int min, int max) {
			super(t);
			ftf = (PJFormattedTextField)getComponent();
		...
		public IntegerEditor(PJTable t, int min, int max, boolean nulleable) {
			super(t);
	...
	private class FloatEditor extends KeyEditor {
		...
		public FloatEditor(PJTable t, float min, float max) {
			super(t);
			ftf = (PJFormattedTextField)getComponent();
	...
	private class DateEditor extends KeyEditor {
		private static final long serialVersionUID = 8134634064643152556L;
		PJFormattedTextField ftf;
		DateListener dateListener;

		public DateEditor(PJTable t) {
			super(t);
			ftf = (PJFormattedTextField)getComponent();
		...
		public boolean stopCellEditing() {
			...
			if(!isValid(s)) {
				PJOptionPane.showWarningMessage(null, "El campo admite una fecha en formato '" + getDateMask() + "'.", "Error");
				return false;
			}
			return super.stopCellEditing();
		}
	...
	private class TimeEditor extends KeyEditor {
		...
		public boolean stopCellEditing() {
			...
			if(!isValid(s)) {
//				PJOptionPane.showWarningMessage(null, "El campo admite una hora en formato 'HH:MM'.", "Error");
				return false;
			}
			return super.stopCellEditing();
		}
	...
	private class StringListener implements KeyListener {
		private PJFormattedTextField ftf;
		private int size;

		public StringListener(PJFormattedTextField ftf, int size) {
			this.ftf = ftf;
			this.size = size;
		}
	...
	private class NumberListener implements KeyListener {
		...
		public NumberListener(PJFormattedTextField ftf, int min, int max) {
			this.ftf = ftf;
			this.max = max;
			this.min = min;
	...
	private class FloatListener implements KeyListener {
		...
		public FloatListener(PJFormattedTextField ftf, float min, float max) {
			this.ftf = ftf;
			this.maxf = max;
			this.minf = min;
	...
	private class DateListener implements KeyListener {
		...
		public DateListener(PJFormattedTextField ftf) {
			this.ftf = ftf;
		}
	...
	private class TimeListener implements KeyListener {
		...
		public TimeListener(PJFormattedTextField ftf) {
			this.ftf = ftf;
		}
	...
	private abstract class KeyEditor extends DefaultCellEditor {
		...
		public KeyEditor(PJTable t) {
			super(new PJFormattedTextField(t));
			clickCountToStart = 1;
		}
		...
		public boolean isCellEditable(EventObject evt) {
			if(evt != null && evt instanceof MouseEvent) {
				...
				lastEditedRow = row;
				((PJFormattedTextField)getComponent()).setText(null);
				return true;
			}
//			row = getSelectedRow();
...
//			setValueAt(null, row, col);
	...
	public abstract class CLColumn {
		...
		/**
		 * Retorna <b>true</b> si la columna est� visible.
		 * @return
		 */
	...
	protected class CLJTableRenderer extends DefaultTableCellRenderer {
		...
		public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected, boolean hasFocus, int row, int col) {
			...
			setOpaque(true);
			Font fuente = ((PJTable)table).getFontCell(row, col);
			if(fuente != null)
				setFont(fuente);
			if(!isSelected) {
	...
	public void sortAllRowsBy(int col, boolean ascending, Comparator comparator) {
		...
		//tabla
//		Hashtable tempCells = new Hashtable(); //Colecci�n temporal que
		//almacena las celdas con su
		//color de fondo
		Hashtable tempRows = new Hashtable(); //Colecci�n temporal que
		//almacena
		//las filas con su color de fondo
		Hashtable tempTipos = new Hashtable();
//		Hashtable tempLocks = new Hashtable(); //Colecci�n temporal que
		//almacena las celdas que estan
		//lockeadas
//		Hashtable tempTooltip = new Hashtable(); //Colecci�n temporal que
		//almacena las celdas con su
		//tooltip
//		Hashtable tempFonts = new Hashtable();
		...
		//Se agrega una columna con el n�mero de fila
...
//						int indexData = ((Integer)((Vector)data.elementAt(k)).elementAt(columnas)).intValue();
	...
	public class ColumnSorter implements Comparator {
		...
		public int compare(Object a, Object b) {
			...
			//Trata las cadenas vac�as como nulls
			if(o1 instanceof String && ((String)o1).length() == 0)
				o1 = null;
			if(o2 instanceof String && ((String)o2).length() == 0)
				o2 = null;
			//Ordena los nulls y los pone �ltimos
			if(o1 == null && o2 == null)
				return 0;
			else if(o1 == null)
				return 1;
	...
	public class CLColumnChooser extends JDialog implements ActionListener {
		...
		private PCheckBoxList listaHeader = new PCheckBoxList();
		...
		public void actionPerformed(ActionEvent evt) {
			...
			if(evt.getSource() == btnAceptar) {
				//Actualiza las columnas de las tablas seg�n las elegidas
	...
	public int getWidthUnused() {
		...
		if(scrollPane != null) {
			...
			if(containerWidth != 0) {
				...
				widthUnused = containerWidth - totalColWidth - PJTable.SCROLL_WIDTH;
			}
		}
		return widthUnused;
	}
	...
	/**
	 * Devuelve el �ndice de la primera fila cuyo valor en la columna pasada
	 * como par�metro es vac�a. Si no existe ninguna devuelve <b>-1</b>.
	 * @param col
	 * @return el indice de la primer fila vac�a en la columna <b>col</b>. Si
	...
	/**
	 * Devuelve el �ndice de la primera fila cuyo valor en la columna pasada
	 * como par�metro es vacio. Empieza a comparar a partir de la fila
	 * <b>filaDesde</b>.
	 * @param filaDesde
	 * @param col
	 * @return el indice de la primer fila vac�a en la columna <b>col</b> y
	...
	/**
	 * Devuelve <b>true</b> si la fila de la tabla est� completa con todos los
	...
	/**
	 * Devuelve el �ndice de la primera fila cuyo valor coindice con
	 * <code>obj</code>.
	 * @param col
	 * @param obj
	 * @return el �ndice de la primera fila. Si no existe devuelve <b>-1</b>.
	 */
     ...
     * La columna DEBE ser de tipo String.
     * @param rowToIgnore Fila a ignorar en la comparaci�n
     * @param col Columna en donde buscar el valor a comparar
     * @param str El String a comparar
     * @param ignoreCase Si se desea comparar ignorando may�sculas y min�sculas
     */
    ...
    /**
	 * Devuelve <b>true</b> cuando existe alguna combinaci�n de valores
	 * repetidos en las columnas pasada como par�metro.
	 * @param columns
     ...
     * @param ignoreCase Ignora may�sculas para las columnas que son de tipo String.
	 ...
	 * Obs: La b�squeda se realiza en forma descendente, o sea que las filas devueltas
	 ...
	 * columnas pasado como par�metro.
	 * @param row
	 * @param columns
	 ...
	 * @param index El �ndice de la columna.
	...
	/**
	 * Al setear una columna como oculta, la misma no aparecer� en el di�logo que
	...
	/**
	 * Setea el formato con el que se muestran los valores num�ricos de la
	...
	public class PJFormattedTextField extends JFormattedTextField {
		...
		public PJFormattedTextField(PJTable t) {
			super();
			this.tabla = t;
				...
				public void keyPressed(KeyEvent evt) {
					if(evt.getKeyCode() == KeyEvent.VK_ESCAPE) {
						...
						if(evt.getKeyCode() == KeyEvent.VK_LEFT || evt.getKeyCode() == KeyEvent.VK_RIGHT) {
							if(PJFormattedTextField.this.getText().length() == 0) {
	 ...
	 * Este m�todo deber� utilizarse despu�s de haber seteado la cantidad
	 * de columnas que contendr� la tabla.
	 * @param tooltipText
	 */
	...
	/**
	 * Agrupa celdas a partir de la celda ubicada en la posici�n
	 * <code>fila</code>,<code>col</code>.<code>alto</code> y
	 * <code>ancho</code> definen el tama�o del grupo el filas y columnas.
	 * @param fila
	 * @param col
	 * @param alto
	 * @param ancho
	...
	/**
	 * Devuelve <b>true</b> si la celda ubicada en la posci�n <code>fila</code>,
	...
	/**
	 * Devuelve el grupo que comienza en la celda ubicada en la posci�n fila,
	 * col.
	 * @param fila
	 * @param col
	 * @return
	...
	/**
	 * Retorna la posici�n x del grupo al que pertenece la celda ubicada en la
	 * posci�n fila, col.
	 * @param fila
	 * @param col
	 * @return
	...
	/**
	 * Retorna la posici�n x del grupo al que pertenece la columna indicada.
	 * @param col
	 * @return
	 */
	...
	/**
	 * Retorna la posici�n y del grupo al que pertenece la celda ubicada en la
	 * posci�n fila, col.
	 * @param fila
	 * @param col
	 * @return
	...
	/**
	 * Devuelve <b>true</b> si la celda ubicada en la posci�n fila, col
	 * pertenece a alg�n grupo.
	 * @param fila
	 * @param col
	 * @return
	 */
	...
	/**
	 * Devuelve <b>true</b> si la columna pertenece a alg�n grupo.
	 * @param col
	 * @return
	 */
	 ...
	 * ubicada en la posici�n fila, col.
	 * @param fila
	 * @param col
	 * @return
	 */
	 ...
	 * celda ubicada en la posici�n fila, col.
	 * @param fila
	 * @param col
	 * @return
	 */
	...
	/**
	 * Devuelve el �ndice de la primera columna del grupo al que pertenece la
	 * columna indicada.
	 * @param col
	 * @return
	 */
	 ...
	 * finalizar cada agrupaci�n inserta <code>cantFilas</code> filas y
	 * devuelve un arreglo del tipo <b>int</b> con los �ndices de las filas
	 * insertadas.
	 * @param colHasta
	 * @return
	 */
	...
	/**
	 * Inserta una fila en la posici�n <code>row</code> con el contenido de
	 * <code>rowData</code>.
	 * @param row
	 * @param rowData
	 */
	...
	/**
	 * Actualiza las distintas colecciones que contienen informaci�n sobre las
	 * celdas.
	 * @param row
	 */
	@SuppressWarnings("unchecked")
	...
	/**
	 * Clase utilizada para la agrupaci�n de celdas.
	 */
	...
	/**
	 * Clase utilizada para la agrupaci�n de headers.
	 */
	...
	public class CLJTableUI extends BasicTableUI {
		...
		/**
		 * M�todo adaptado para que soporte la agrupaci�n celdas.
		 * @param g
		 * @param cellRect
		 * @param row
		 * @param column
		...
		/**
		 * M�todo que pinta las celdas de acuerdo a si pertenecen a una columna
	...
	/**
	 * Devuelve un vector con los datos necesarios para generar un gr�fico de
	 * torta.
	 * @param filaDesde
	 * @param filaHasta
	 * @param colCategoria
	...
	/**
	 * Crea un header m�ltiple para las columnas especificadas.
	 * @param nombre
	 * @param colInicio
	 * @param colFin
	 */
	...
	/**
	 * Retorna si se permite o no el uso de headers m�ltiples.
	...
	protected class PopupMenu extends JPopupMenu {
		public PopupMenu() {
			if(permiteCopiar) {
				...
				add(copiarTodo);
				JMenuItem copiarSeleccion = new JMenuItem("Copiar selecci�n");
...
//		String valor;
	...
	/** Clase para poder utilizar headers m�ltiples */
...
//				int columnMargin = header.getColumnModel().getColumnMargin();
		...
		/**
		 * M�todo adaptado para soportar la agrupaci�n de headers y headers
		 * m�ltiples.
		 * @param g
		 * @param cellRect
		 * @param columnIndex
		...
		private void paintCell(Graphics g, Rectangle cellRect, int columnIndex) {
			PJTable tabla = (PJTable)header.getTable();
		...
		/**
		 * M�todo para pintar header m�ltiples.
		 * @param g
		 * @param cellRect
		 * @param cGroup
		 */
		...
		/**
		 * M�todo paintCell de BasicTableHeaderUI.
		 * @param g
		 * @param cellRect
		 * @param columnIndex
		 */
		...
		/**
		 * M�todo adaptado para soportar mulptiples headers.
		 * @return
		 */
