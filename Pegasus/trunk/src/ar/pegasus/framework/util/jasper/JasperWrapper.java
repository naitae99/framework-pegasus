...
/**
 * Clase útil para el sencillo manejo del paquete <b>Jasper Reports</b> (ver
	...
	public static boolean impresionComprobante(JasperWrapperProperties properties, boolean preview, int idImpresora, String titulo, Dimension dimension) {
		try {
			...
			if (e.getCause() != null && e.getCause().getMessage().equals("Printer is not accepting job.")) {
				BossError.gestionarError(new PException(BossError.ERR_CONEXION, "No se puede imprimir", "La Impresora no responde o no está accesible", e, new String[] {"Verifique el estado de la impresora"}));
			} else {
				BossError.gestionarError(new PException(BossError.ERR_CONEXION, "No se puede imprimir", "La Impresora no responde o no está accesible", e, null));				
			}
			return false;
		}
		catch (Exception e) {
