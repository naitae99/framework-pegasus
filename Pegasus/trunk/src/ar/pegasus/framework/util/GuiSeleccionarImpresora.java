...
public class GuiSeleccionarImpresora extends JDialog {
	private static final long serialVersionUID = 7557310243521983004L;
	 ...
	 * @see GuiSeleccionarImpresora(Frame owner, ListPrinterMode printMode)
	 */
	@Deprecated
	...
	private boolean validarResolucion() {
		...
		if(chkImpresoraMatrizPunto.isSelected()) {
			if(resolution[0] > MIN_CROSSFEED_RESOLUTION_DPI || resolution[1] > MIN_FEED_RESOLUTION_DPI) {
				if(PJOptionPane.showQuestionMessage(getOwner(), "La impresora seleccionada podría no ser de matriz de punto. ¿Desea continuar?", "Advertencia") == PJOptionPane.NO_OPTION) {
			...
			if(resolution[0] < MIN_CROSSFEED_RESOLUTION_DPI || resolution[1] < MIN_FEED_RESOLUTION_DPI) {
				if(PJOptionPane.showQuestionMessage(getOwner(), "La impresora seleccionada aparenta ser de matriz de punto. ¿Desea continuar?", "Advertencia") == PJOptionPane.YES_OPTION) {
