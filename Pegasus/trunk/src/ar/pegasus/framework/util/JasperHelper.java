...
import ar.pegasus.framework.boss.ElementoListado;
...
public class JasperHelper {
	...
	public static Integer imprimirReporte(JasperPrint reporte, boolean conDialogoImpresion, boolean esperarPorCopia, int copias) throws JRException {
		...
		for (int i = 0; i < copias; i++) {
			...
			if (esperarPorCopia && (i+1)<copias) {
				if (PJOptionPane.showQuestionMessage(null, "Continuar?", "Impresión") == PJOptionPane.NO_OPTION) {
					break;
				}
			}
		}
	...
	public static void imprimirListado(PJTable tabla, String titulo, String subtitulo, String filtros, boolean confirmacion) {
	...
	public static void listadoAPDF(PJTable tabla, String titulo, String subtitulo, String filtros, boolean confirmacion,String path) {
		if( tabla.getRowCount()>0) {
			try {
				...
				BossError.gestionarError(new PException("No se pudo convertir a PDF"));
			}
		}
	}
...
//		parameters.put("Usuario", GTLGlobalCache.getInstance().getUsuarioSistema().getUsrName());
	...
	private static List<ElementoListado> getData(PJTable tabla) {
